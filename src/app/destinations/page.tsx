      ...
      <header className="fixed top-0 w-full z-50 safe-top">
        <div className="glass px-4 py-3 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-6 text-sm">
            <Link href="/yachts" className="text-gray-300 hover:text-white transition">Yachts</Link>
            <Link href="/destinations" className="text-white font-semibold">Destinations</Link>
        ...
        <div className="relative h-56 md:h-72 overflow-hidden">
          ...
          <div className="relative z-10 h-full flex flex-col items-center justify-center px-4 text-center">
            <div className="inline-flex items-center gap-2 glass px-4 py-2 rounded-full mb-4">
              ...
              <span className="text-xs text-gray-300">Turkish Riviera</span>
            </div>
            ...
            {destinations.map(dest => (
              <button
                ...
                className={`relative rounded-2xl overflow-hidden h-32 md:h-40 group transition-all ${
                  selectedDestination.id === dest.id ? 'ring-2 ring-blue-400 scale-[1.02]' : ''
                }`}
              >
          ...
          <div className="glass rounded-3xl overflow-hidden mb-8">
            ...
            <div className="relative h-64 md:h-96">
              ...
              <div className="absolute bottom-6 left-6 right-6">
                <p className="text-sm text-blue-300 mb-1">{selectedDestination.region}</p>
            ...
            <div className="p-6 md:p-8">
              {/* Description */}
              <p className="text-gray-400 leading-relaxed mb-8">{selectedDestination.description}</p>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
                ...
                <div className={`rounded-2xl p-4 text-center bg-gradient-to-br ${selectedDestination.color}`}>
                  ...
                  <p className="text-xs text-gray-400">Sunny Days</p>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {selectedDestination.routes.map(route => (
                    <div key={route.name} className="glass rounded-2xl p-5 hover:border-blue-500/30 transition">
                      <div className="flex items-center justify-between mb-3">
                        <h4 className="font-semibold">{route.name}</h4>
                        <span className="glass px-2.5 py-1 rounded-full text-xs text-blue-300 flex items-center gap-1">
                        ...
                        {route.stops.map((stop, idx) => (
                          <div key={stop} className="flex items-center gap-3">
                            <div className="flex flex-col items-center">
                              <div className={`w-3 h-3 rounded-full ${idx === 0 ? 'bg-blue-400' : idx === route.stops.length - 1 ? 'bg-green-400' : 'bg-white/30'}`} />
                              {idx < route.stops.length - 1 && <div className="w-0.5 h-4 bg-white/10" />}
                            </div>
                      ...
                      <Link
                        ...
                        className="mt-4 text-xs text-blue-400 flex items-center gap-1 hover:text-blue-300 transition"
                      >
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                <Link
                  href={`/yachts?destination=${selectedDestination.name}`}
                  className="flex-1 bg-blue-600 hover:bg-blue-700 py-3 rounded-xl font-semibold text-sm transition text-center flex items-center justify-center gap-2"
                >
                  <Ship className="w-4 h-4" /> Browse Yachts in {selectedDestination.name}
                </Link>
                <Link
                  href="/#contact"
                  className="flex-1 glass py-3 rounded-xl font-semibold text-sm hover:bg-white/10 transition text-center"
                >
                  Ask Our Experts
                </Link>
              </div>
          ...
          <div className="glass rounded-3xl p-6 md:p-8 mb-8">
                ...
                return (
                  <div key={month} className="text-center">
                    <div className={`h-20 md:h-28 rounded-lg mb-1 flex items-end justify-center pb-2 transition ${
                      isPeak ? 'bg-gradient-to-t from-blue-600/40 to-blue-400/20' :
                      isShoulder ? 'bg-gradient-to-t from-blue-600/20 to-blue-400/10' :
                      'bg-white/5'
                    }`}>
                      <div className={`w-full mx-1 rounded-t-sm ${
                        isPeak ? 'bg-blue-500 h-full' :
                        isShoulder ? 'bg-blue-500/50 h-2/3' :
                        'bg-white/10 h-1/4'
                      }`} />
                    </div>
            ...
            <div className="flex items-center justify-center gap-6 text-xs">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-400">Off Season</span>
              </div>
            </div>
          </div>
        </div>
