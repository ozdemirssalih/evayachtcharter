      ...
      <div className="min-h-screen bg-white flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-gray-600 text-sm mb-6">
            Your reservation for <span className="text-gray-800 font-semibold">{yacht.name}</span> has been confirmed.
          ...
          <div className="glass rounded-2xl p-5 mb-6 text-left">
            <div className="space-y-3">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-mono font-bold text-teal-500">EVA-{Date.now().toString(36).toUpperCase()}</span>
              </div>
              <div className="flex justify-between text-sm">
              ...
              <div className="flex justify-between text-sm font-bold pt-2 border-t border-teal-200">
                <span>Total Paid</span>
                <span className="text-teal-500">&euro;{totalPrice.toLocaleString()}</span>
              </div>
            </div>
          </div>
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="fixed top-0 w-full z-50 safe-top">
        <div className="glass px-4 py-3 flex items-center justify-between">
          <Link href={`/yachts/${yachtId}`} className="flex items-center gap-2 text-gray-600 hover:text-gray-800 transition">
            <ChevronLeft className="w-5 h-5" />
            <span className="text-sm">Back</span>
          </Link>
          <span className="text-sm font-semibold">Booking</span>
          <span className="text-sm text-gray-600">Step {step}/4</span>
        </div>
      </header>
          ...
          {[
            ...
            <div key={s.n} className="flex-1">
              <div className="flex items-center gap-2">
                ...
                {idx < 3 && (
                  <div className={`flex-1 h-0.5 rounded-full transition ${step > s.n ? 'bg-teal-600' : 'bg-teal-50'}`} />
                )}
              </div>
              <p className={`text-[10px] mt-1 ${step >= s.n ? 'text-teal-500' : 'text-gray-600'}`}>{s.label}</p>
            </div>
          ))}
        </div>
          ...
          <div className="lg:col-span-2">
              ...
              <div className="space-y-6">
                <div className="glass rounded-2xl p-6">
                  <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
                    <Calendar className="w-5 h-5 text-teal-500" /> Select Dates
                  </h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs text-gray-600 mb-1 block">Check-in Date</label>
                      <input
                        type="date"
                        value={dates.start}
                        onChange={e => setDates(prev => ({ ...prev, start: e.target.value }))}
                        className="w-full bg-gray-50 border border-teal-200 rounded-xl px-4 py-3 text-sm outline-none text-gray-800 focus:border-teal-500 transition"
                      />
                    </div>
                    <div>
                      <label className="text-xs text-gray-600 mb-1 block">Check-out Date</label>
                      <input
                        type="date"
                        value={dates.end}
                  ...
                  {days > 0 && (
                    <div className="mt-3 flex items-center gap-2 text-sm text-teal-500">
                ...
                <div className="glass rounded-2xl p-6">
                  <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
                    <Users className="w-5 h-5 text-teal-500" /> Number of Guests
                  </h2>
                  <div className="flex items-center gap-4">
                    <button
                      onClick={() => setGuests(Math.max(1, guests - 1))}
                      className="w-10 h-10 rounded-full glass flex items-center justify-center text-lg hover:bg-teal-50 transition"
                    >
                      -
                    </button>
                    <div className="text-center">
                      <p className="text-3xl font-bold">{guests}</p>
                      <p className="text-xs text-gray-600">guest{guests > 1 ? 's' : ''}</p>
                    </div>
                    <button
                  ...
                  <p className="text-xs text-gray-600 mt-2">Maximum {yacht.guests} guests</p>
                </div>
              </div>
            )}
            ...
            {step === 2 && (
              <div className="space-y-4">
                ...
                <p className="text-sm text-gray-600 mb-4">Add optional services to make your trip unforgettable</p>
                {extras.map(extra => (
                  <button
                    key={extra.id}
                    onClick={() => toggleExtra(extra.id)}
                    className={`w-full glass rounded-2xl p-5 flex items-center gap-4 text-left transition ${
                      selectedExtras.includes(extra.id) ? 'border-teal-500/50 bg-teal-500/10' : 'hover:bg-gray-50'
                    }`}
                  >
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-gray-600">{extra.desc}</p>
                    </div>
                    <div className="text-right">
                      <p className="font-bold text-teal-500">&euro;{extra.price}</p>
            ...
            {step === 3 && (
              <div className="glass rounded-2xl p-6">
                <h2 className="text-lg font-bold mb-4 flex items-center gap-2">
                  <User className="w-5 h-5 text-teal-500" /> Contact Information
                </h2>
                <div className="space-y-4">
                  <div>
                  ...
                  <div>
                    <label className="text-xs text-gray-600 mb-1 block">Special Requests</label>
                    <textarea
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-2 bg-gray-50 border border-teal-200 rounded-xl px-3 py-3">
                        ...
                        <input
                          ...
                          className="bg-transparent text-sm w-full outline-none text-gray-800 font-mono"
                        />
                      </div>
                    </div>
                    <div>
                      ...
                      <input
                        ...
                        className="w-full bg-gray-50 border border-teal-200 rounded-xl px-4 py-3 text-sm outline-none text-gray-800 focus:border-teal-500 transition font-mono"
                      />
                    </div>
                ...
                <div className="glass rounded-2xl p-6 bg-gradient-to-r from-green-500/10 to-emerald-500/10 border-green-500/20">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-600">Your payment is processed securely with 256-bit SSL encryption. We never store your card details.</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="glass rounded-2xl p-5 sticky top-20">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600">{yacht.type}</p>
                  <div className="flex items-center gap-1 mt-0.5">
                    <Star className="w-3 h-3 text-yellow-400 fill-yellow-400" />
                    <span className="text-xs">{yacht.rating}</span>
                    <span className="text-xs text-gray-600">| <MapPin className="w-3 h-3 inline" /> {yacht.destination}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="border-t border-teal-200 pt-4 space-y-3">
                {dates.start && dates.end && (
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="text-xs text-teal-500">{days} days</span>
                  </div>
                )}
                ...
                {days > 0 && (
                  <>
                          ...
                          {selectedExtras.map(id => {
                            ...
                            return (
                              <div key={id} className="flex justify-between text-sm">
                                <span className="text-gray-600">{extra.name} x {days}d</span>
                    ...
                    <div className="flex justify-between font-bold text-lg pt-3 border-t border-teal-200">
                      <span>Total</span>
        ...
        <div className="max-w-4xl mx-auto flex items-center justify-between px-4 py-3">
          {step > 1 ? (
            <button
              ...
              className="glass px-5 py-2.5 rounded-xl text-sm font-semibold hover:bg-teal-50 transition flex items-center gap-1"
            >
